/**
 * pi AGENTS.md — creates AGENTS.md from softisp template if missing.
 */
...
/**
 * Default AGENTS.md template based on softisp's autonomous agent directive.
 * This template enforces strict operational rules for the session.
 */
function buildAgentsMdContent(createdDate: string): string {
  return `# Autonomous Engineering Agent Operational Directive
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event: any, ctx: any) => {
    ...
    const content = buildAgentsMdContent(createdDate);
    await fs.writeFile(agentsMdPath, content, "utf-8");
    ctx.ui.notify(`Created AGENTS.md from softisp template`, "info");
  });
}